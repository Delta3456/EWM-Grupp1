...
from geometry import extract_geometry_from_step
...
def main():
    ...
    t1 = time.time()
    r, c0, phi0 = extract_geometry_from_step(
        cfg['step_file'], cfg['R'], n_sections=cfg.get('n_sections', 30)
    )
    ...
    # 4) Airfoil-Screening
    t2 = time.time()
    polars = read_polars(cfg['airfoil_dir'])
    v_target = cfg['v_target_factor'] * v_opt_orig
    mid_idx = len(r) // 2
    Re_target = v_target * c0[mid_idx] / 1.5e-5
    selected = select_airfoils(polars, cfg, Re_target)
